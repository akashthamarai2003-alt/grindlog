...
export default function SignInPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await signIn(cleanEmail, form.password);
    setIsLoading(false);
    ...
    if (result.success) {
      ...
      if (rawError.toLowerCase().includes("invalid login credentials")) {
        setError("Account not found or password incorrect. If this account was deleted, please click Signup to register.");
      } else {
        setError(rawError || "Invalid login credentials. Please try again.");
      }
    }
  };
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <motion.button
              type="submit"
              whileTap={{ scale: 0.96 }}
              disabled={isLoading || !cleanEmail || !form.password || (touched.email && !isValidEmail)}
