...
export async function updateUserPlanAdminAction({
  ...
  try {
    ...
    if (isPremium) {
      const d = new Date();
      if (premiumTier === "monthly") d.setMonth(d.getMonth() + 1);
      else if (premiumTier === "six_months") d.setMonth(d.getMonth() + 6);
      else if (premiumTier === "lifetime") d.setFullYear(d.getFullYear() + 100);
    ...
    const { error: profileError } = await supabaseAdmin
      ...
      .update({
        ...
        premium_level: isPremium ? premiumLevel : "core",
        premium_tier: isPremium ? premiumTier : "monthly",
    ...
    if (profileError) {
      ...
      return { success: false, error: profileError.message || "Failed to update profile plan" };
    }

    if (customPaidAmount !== undefined && customPaidAmount >= 0) {
      const { error: subError } = await supabaseAdmin.from("subscriptions").insert({
        user_id: userId,
        plan: `admin_manual_${premiumTier}_${premiumLevel}`,
        status: isPremium ? "active" : "cancelled",
        razorpay_payment_id: `pay_manual_₹${customPaidAmount}_${Date.now()}`,
      });

      if (subError) {
        console.warn("Manual subscription insert notice:", subError);
      }
    }
    ...
    console.error("updateUserPlanAdminAction error:", error);
