...
import { 
  Loader2, AlertTriangle, ArrowRight, Brain, Dumbbell, Apple, Droplets, Flame, 
  ShoppingCart, ShieldAlert, HeartPulse, CalendarDays, CircleCheck, Activity, 
  Sparkles, Utensils, Store, ShoppingBag, CheckCircle2 
} from 'lucide-react';
...
function foodRoutineLabel(foodEnvironment: unknown): string | null {
  ...
  if (/cook/i.test(environment)) return "I Cook meals";
  return `${environment} meals`;
}

function DietShowcasePreview({
  planData,
  onReviewWorkouts,
}: {
  planData: any;
  onReviewWorkouts: () => void;
}) {
  const profile = planData?._profile;
  const nutrition = planData?.nutrition;
  const foodEnvironment = typeof profile?.food_environment === "string" ? profile.food_environment : "";
  const nutritionGoal = String(planData?.plan?.goal || profile?.goal || "").trim();
  ...
  const nutritionMealCountLabel = Number.isFinite(nutritionMealCount) && nutritionMealCount > 0
    ...
    : String(profile?.meals_per_day || "").trim();
  ...
  return (
    <div className="mx-auto max-w-md px-6 pb-5 space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
      {/* Top Personalized Plan Card */}
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="rounded-2xl border border-[#1A2619] bg-[#121E12] p-4 text-center">
          ...
          <span className="text-xl font-black text-white">{nutrition?.daily_calories || '--'}</span>
        </div>
        <div className="rounded-2xl border border-[#1A2619] bg-[#121E12] p-4 text-center">
          ...
          <span className="text-xl font-black text-white">{nutrition?.protein_grams ? `${nutrition.protein_grams}g` : '--'}</span>
        </div>
        <div className="rounded-2xl border border-[#1A2619] bg-[#121E12] p-4 text-center">
          ...
          <span className="text-xl font-black text-white">{nutrition?.carbs_grams ? `${nutrition.carbs_grams}g` : '--'}</span>
        </div>
        <div className="rounded-2xl border border-[#1A2619] bg-[#121E12] p-4 text-center">
          ...
          <span className="text-xl font-black text-white">{nutrition?.fat_grams ? `${nutrition.fat_grams}g` : '--'}</span>
        </div>
      </div>

      {/* Notice Card for Full Plan in Nutrition Page */}
      <div className="rounded-3xl border border-[#1A2619] bg-[#121E12] p-6 text-center space-y-3">
        ...
        <div className="pt-2">
          <button
            type="button"
            onClick={onReviewWorkouts}
            className="inline-flex items-center gap-2 px-5 py-2.5 bg-[#1A2619] hover:bg-[#233522] border border-[#2A3F29] text-white rounded-full text-xs font-black uppercase tracking-wider transition-all"
          >
            <Dumbbell size={14} className="text-[#ADFF00]" />
            <span>Review AI Workout Plan</span>
...
function GroceryShowcasePreview({
  planData,
  onReviewWorkouts,
}: {
  planData: any;
  ...
  const profile = planData?._profile;
  const foodEnvironment = typeof profile?.food_environment === "string" ? profile.food_environment : "";
  const isProvided = usesProvidedCoreMeals(profile);

  return (
    <div className="mx-auto max-w-md px-6 pb-5 space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
      {/* Top Grocery Showcase Card */}
      <section className="overflow-hidden rounded-3xl border border-[#ADFF00]/20 bg-[linear-gradient(145deg,rgba(173,255,0,0.10),rgba(18,30,18,1)_45%)] p-5">
        ...
        <p className="mt-1 text-sm leading-relaxed text-gray-400">
          {isProvided ? `Only food to buy outside your ${foodEnvironment} meals.` : 'A 30-day shopping list built from the foods and budget you saved.'}
        </p>
      ...
      {/* Notice Card for Full Grocery in Nutrition Page */}
      <div className="rounded-3xl border border-[#1A2619] bg-[#121E12] p-6 text-center space-y-3">
        <div className="w-12 h-12 rounded-2xl bg-[#ADFF00]/10 border border-[#ADFF00]/20 flex items-center justify-center mx-auto text-[#ADFF00]">
