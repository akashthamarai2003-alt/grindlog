...
import { ArrowLeft, Check, Loader2, Dumbbell, Scale, Target, Flame, Heart, Info, ChevronRight } from "lucide-react";
...
export function OnboardingFlow({ initialData = {} }: { initialData?: Partial<OnboardingData> }) {
  ...
  const handleNext = () => {
    // We could run Zod client-side validation per step here, but for smooth UX
    // we'll validate the specific fields required for the step if needed.
    // For now, we just proceed. We will validate completely on save.
  ...
  const OptionCard = ({ 
    ...
    >
      {Icon && (
        <div className={`p-3 rounded-full mr-4 ${selected ? "bg-green-100 text-green-600" : "bg-gray-100 text-gray-500"}`}>
          <Icon size={24} />
        </div>
      )}
      <div className="flex-1">
        <h3 className={`font-semibold text-lg ${selected ? "text-green-700" : "text-gray-900"}`}>{title}</h3>
        {desc && <p className={`text-sm mt-1 ${selected ? "text-green-600/80" : "text-gray-500"}`}>{desc}</p>}
      </div>
      {selected && (
        <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="text-green-500 ml-4">
          <Check size={24} />
        </motion.div>
      )}
    </motion.button>
  ...
  const BottomBar = ({ 
    ...
    <div className="fixed bottom-0 left-0 right-0 max-w-[390px] mx-auto p-4 bg-white/80 backdrop-blur-md border-t border-gray-100 pb-safe">
      <button
        disabled={!canProceed}
        onClick={onProceed}
        className={`w-full py-4 rounded-xl font-bold text-lg transition-all ${
          canProceed 
            ? "bg-green-500 text-white hover:bg-green-600 active:scale-[0.98] shadow-lg shadow-green-500/25" 
  ...
  const renderStep = () => {
    switch(step) {
      case 1:
        return (
          <div className="flex flex-col h-[80vh] justify-center px-6">
            <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="flex-1 flex flex-col justify-center">
              <div className="w-16 h-16 bg-green-100 rounded-3xl flex items-center justify-center mb-6 text-green-500">
                <Target size={32} />
              </div>
              ...
              <p className="text-lg text-gray-500">
                Tell us about yourself and we'll build your fitness experience around you.
              </p>
            </motion.div>
            <div className="pb-8 space-y-4">
              <button onClick={handleNext} className="w-full py-4 bg-green-500 text-white rounded-xl font-bold text-lg shadow-lg shadow-green-500/25 active:scale-[0.98] transition-transform">
                Let's Get Started
              </button>
              <button onClick={() => router.push("/fitness")} className="w-full py-4 text-gray-500 font-medium active:scale-[0.98] transition-transform">
            ...
            <div className="space-y-6">
              <div>
                ...
                <input 
                  ...
                  className="w-full p-4 rounded-xl border-2 border-gray-200 focus:border-green-500 focus:ring-0 text-lg transition-colors outline-none" 
                  placeholder="e.g. 25"
                />
              </div>
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Target Weight (kg) <span className="text-gray-400 font-normal">(Optional)</span></label>
                <input 
                  type="number" 
              ...
              <div>
                ...
                <select 
                  ...
                  className="w-full p-4 rounded-xl border-2 border-gray-200 focus:border-green-500 focus:ring-0 text-lg transition-colors outline-none bg-white appearance-none"
                >
        ...
        return (
          <div className="px-6 pt-6 pb-32">
            ...
            <div className="space-y-8">
                  ...
                  {[2,3,4,5,6,7].map(d => (
                    <button 
                      ...
                      className={`w-12 h-12 rounded-xl flex items-center justify-center text-lg font-bold transition-all border-2 ${
                        data.training_days_per_week === d ? 'bg-green-500 border-green-500 text-white' : 'border-gray-200 text-gray-600 bg-white'
                      }`}
                    >
                      {d}
                    </button>
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-3">Preferred Workout Duration</label>
                  ...
                  {[10, 20, 30, 45, 60, 90].map(m => (
                    <button 
                      ...
                      className={`py-3 rounded-xl flex flex-col items-center justify-center transition-all border-2 ${
                        data.workout_duration_minutes === m ? 'bg-green-50 border-green-500 text-green-700' : 'border-gray-200 text-gray-600 bg-white'
                      }`}
                    >
      ...
      case 9:
        return (
          <div className="px-6 pt-6 pb-32">
            ...
            <div className="space-y-6">
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-3">Daily Activity Level</label>
                <div className="space-y-3">
                  {[
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">Average Sleep (Hours)</label>
                <input 
                  type="number" 
                  min={3} max={16}
          ...
          <div className="px-6 pt-6 pb-32">
            ...
            <div className="space-y-6">
              {[
                ...
                { label: "Training Location", value: data.training_location, stepIndex: 5 },
                { label: "Equipment", value: data.equipment?.join(", "), stepIndex: 6 },
                { label: "Training Schedule", value: `${data.training_days_per_week} days/wk, ${data.workout_duration_minutes} min/day`, stepIndex: 7 },
                { label: "Nutrition", value: `${data.diet_preference}, ${data.meals_per_day} meals/day`, stepIndex: 8 },
                ...
                <div key={idx} className="bg-gray-50 p-4 rounded-2xl border border-gray-100 flex justify-between items-center">
                  <div className="pr-4">
                    <p className="text-sm font-semibold text-gray-500 mb-1">{section.label}</p>
                    <p className="text-gray-900 font-medium">{section.value || "Not set"}</p>
                  </div>
                  <button 
                    onClick={() => jumpToStep(section.stepIndex)}
                    className="p-2 text-green-600 bg-green-100 rounded-lg hover:bg-green-200 active:scale-95 transition-all flex-shrink-0"
                  >
                    Edit
                  </button>
                </div>
      ...
      case 11:
        return (
          <div className="flex flex-col h-[80vh] justify-center px-6 text-center">
            <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="flex-1 flex flex-col justify-center items-center">
              <div className="w-20 h-20 bg-green-500 rounded-full flex items-center justify-center mb-6 text-white shadow-xl shadow-green-500/30">
                <Check size={40} />
              </div>
              <h1 className="text-4xl font-bold text-gray-900 tracking-tight leading-tight mb-4">
                You're ready.
              </h1>
              <p className="text-lg text-gray-500 max-w-[280px]">
            ...
            <div className="pb-8">
              <button 
                ...
                disabled={isSaving}
                className="w-full py-4 bg-green-500 text-white rounded-xl font-bold text-lg shadow-lg shadow-green-500/25 active:scale-[0.98] transition-transform flex items-center justify-center"
              >
                {isSaving ? <Loader2 className="animate-spin" /> : "Enter My Fitness OS"}
              </button>
            </div>
          </div>
        );
  ...
  return (
    <div className="min-h-[100dvh] w-full max-w-[390px] mx-auto bg-white flex flex-col relative overflow-hidden">
