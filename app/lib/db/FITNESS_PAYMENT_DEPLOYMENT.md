...
1. Apply `fitness_payment_settlement.sql` to the target Supabase database before deploying the application. It adds a receipt ledger, paid-amount and locked-rate columns, and a service-role-only settlement transaction. No migration has been applied by this task.
...
3. In Razorpay test mode, verify Core and Pro renewal; expired and active membership; callback before webhook and webhook before callback; repeat both deliveries; checkout cancellation; failed payment; delayed webhook; reload/UPI return. Check that a first wheel purchase saves its captured price, legacy wheel purchases recover their price from Razorpay, and future orders charge that same price for the same plan. A payment must produce exactly one receipt and one expiry extension. Confirm an unrelated account or different order cannot activate access.
