-- ============================================
-- Fitness AI OS — Database Migration Phase 8 (Fix Plan Data)
-- ============================================

-- 1. Add plan_data column to store the raw AI payload
ALTER TABLE fitness_os_workout_plans 
ADD COLUMN IF NOT EXISTS plan_data JSONB;

-- 2. Update the RPC to insert the payload into plan_data
...
BEGIN
  ...
  -- 1. Create Plan (NOW SAVING plan_data AS WELL!)
  INSERT INTO fitness_os_workout_plans (user_id, name, description, goal, status, plan_data)
  VALUES (
    v_user_id,
    payload->'plan'->>'name',
