...
alter table public.fitness_os_subscriptions add column if not exists locked_rate_paise bigint;
alter table public.fitness_os_subscriptions add column if not exists locked_level text;
...
create or replace function public.settle_fitness_payment(p_user uuid, p_order text, p_payment text, p_tier text, p_level text, p_amount bigint, p_currency text, p_lock boolean)
returns void language plpgsql security definer set search_path = public as $$
declare base_expiry timestamptz; new_expiry timestamptz; previous_rate bigint; previous_level text; new_rate bigint;
begin
 ...
 select locked_rate_paise, locked_level into previous_rate, previous_level
 from fitness_os_subscriptions where user_id=p_user;
 new_rate := case
   when previous_level=p_level and previous_rate > 0 then previous_rate
   when p_lock then p_amount
   else null
 end;
 ...
 insert into fitness_os_subscriptions(user_id,plan,status,provider,provider_order_id,provider_payment_id,current_period_start,current_period_end,locked_rate_paise,locked_level)
 values(p_user,case when p_level='pro' then 'pro' else 'starter' end,'active','razorpay',p_order,p_payment,now(),new_expiry,new_rate,case when new_rate is not null then p_level else null end)
 on conflict(user_id) do update set plan=excluded.plan,status=excluded.status,provider=excluded.provider,provider_order_id=excluded.provider_order_id,provider_payment_id=excluded.provider_payment_id,current_period_start=excluded.current_period_start,current_period_end=excluded.current_period_end,locked_rate_paise=excluded.locked_rate_paise,locked_level=excluded.locked_level;
...
$$;
revoke all on function public.settle_fitness_payment(uuid,text,text,text,text,bigint,text,boolean) from public,anon,authenticated;
grant execute on function public.settle_fitness_payment(uuid,text,text,text,text,bigint,text,boolean) to service_role;
