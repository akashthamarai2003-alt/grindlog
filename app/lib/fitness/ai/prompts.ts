...
3. Generate 'exercises' for each workout that fit within my '${profile.workout_duration_minutes || 45} minute' duration and match my ${profile.training_location} / ${Array.isArray(profile.equipment) ? profile.equipment.join(", ") : "None"} constraints. 
   ...
   - CRITICAL EQUIPMENT RULE: You MUST strictly prescribe exercises based on the 'Available Equipment' list! If my location is 'Home' and equipment is 'None', program bodyweight-only exercises. If my equipment is just 'Dumbbells', program ONLY dumbbell exercises. Even if 'Gym' is selected, if a specific machine like 'Treadmill' is NOT in my 'Available Equipment' list, you MUST NOT prescribe it! For cardio, if 'Treadmill' is missing, suggest 'Brisk Walking (Outdoors)', 'Jogging', or 'Walking' instead.
...
4. Generate 'nutrition' providing a safe daily_calories target and protein_grams. Create a 'meals' array reflecting my ${profile.meals_per_day || "3 meals"} preference. For each meal, provide specific, realistic food items that fit my Budget (${profile.nutrition_budget || "Not specified"}), Diet (${profile.food_type || profile.diet_preference}), and Lifestyle (${profile.lifestyle_description}). 
   ...
   - CRITICAL MEALS PER DAY RULE: You MUST generate EXACTLY the number of meals matching my meals_per_day preference (${profile.meals_per_day || '3 meals'}). If '2 meals': generate only Lunch and Dinner. If '3 meals': generate Breakfast, Lunch, Dinner. If '4 meals': generate Breakfast, Lunch, Snack, Dinner. If '5+ meals': generate Breakfast, Mid-Morning Snack, Lunch, Evening Snack, Dinner.
