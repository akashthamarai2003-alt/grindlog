...
3. Generate 'exercises' for each workout that fit within my '${profile.workout_duration_minutes || 45} minute' duration and match my ${profile.training_location} / ${Array.isArray(profile.equipment) ? profile.equipment.join(", ") : "None"} constraints. 
   - CRITICAL PROGRESSION RULE: Rep ranges MUST match my goal ('${profile.goal || "Not specified"}'). If 'Build Strength', program 3-6 reps for compound lifts. If 'Build Muscle', program 8-12 reps. If 'Lose Fat' or 'Cut', program mostly 8-15 controlled reps; Cut must retain progressive resistance training and must not become an excessive-cardio or starvation plan.
...
4. Generate 'nutrition' providing a safe daily_calories target and protein_grams. Create a 'meals' array reflecting my ${profile.meals_per_day || "3 meals"} preference. For each meal, provide specific, realistic food items that fit my Budget (${profile.nutrition_budget || "Not specified"}), Diet (${profile.food_type || profile.diet_preference}), and Lifestyle (${profile.lifestyle_description}). 
   - CRITICAL DIET RULE: You MUST strictly adhere to my Diet Preference (${profile.food_type || profile.diet_preference}). If Non-Vegetarian, eggs, chicken, fish, and meat are fully allowed and encouraged for protein. If Eggetarian, eggs and dairy are allowed, but do NOT suggest meat or fish. If Vegetarian (Veg), DO NOT suggest ANY meat, fish, or eggs (use paneer, curd, soy chunks, lentils). If Vegan, DO NOT suggest ANY dairy, eggs, whey protein, or meat (use tofu, soy, legumes). If Jain, avoid onions and garlic.
