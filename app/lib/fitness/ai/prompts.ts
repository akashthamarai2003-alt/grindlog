...
${(!profile.equipment || !profile.equipment.includes('Treadmill')) ? `   - ABSOLUTE CARDIO RULE: The user DOES NOT have a Treadmill. You are STRICTLY FORBIDDEN from generating any exercise containing the word 'Treadmill'. If you program cardio, use exactly 'Brisk Walking', 'Jogging (Outdoors)', or 'Walking'. NEVER output 'Treadmill'.` : ''}
...
4. Generate 'nutrition' providing a safe daily_calories target and protein_grams. Create a 'meals' array reflecting my ${profile.meals_per_day || "3 meals"} preference. For each meal, provide specific, realistic food items that fit my Budget (${profile.nutrition_budget || "Not specified"}), Diet (${profile.food_type || profile.diet_preference}), and Lifestyle (${profile.lifestyle_description}). 
   ...
   - CRITICAL FOOD ENVIRONMENT RULE:
     * If Food Environment is 'PG', 'Hostel', 'Home', or 'Office/Canteen': The user's CORE MEALS (breakfast, lunch, dinner) are ALREADY PROVIDED for free by their PG/hostel/family/canteen. Do NOT suggest specific dishes for core meals — instead, the first item in the 'items' array MUST be exactly "PG/Home Provided Meal (Rice/Dal/Roti/etc) - Cost: ₹0". Then, add cheap, no-cook protein add-ons as the next items in the array (e.g., "Add-on: 2 Boiled Eggs - Cost: ₹12"). The user's budget (${profile.nutrition_budget || 'Not specified'}) is ONLY for these add-ons. Total add-on cost MUST stay within budget.
