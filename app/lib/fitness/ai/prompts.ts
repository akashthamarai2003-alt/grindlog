...
${(!profile.equipment || !profile.equipment.includes('Treadmill')) ? `   - ABSOLUTE CARDIO RULE: The user DOES NOT have a Treadmill. You are STRICTLY FORBIDDEN from generating any exercise containing the word 'Treadmill'. DO NOT name any workout 'CARDIO DAY' as it implies a treadmill session. If you must program cardio, name the workout 'ACTIVE RECOVERY (WALKING)' and use exactly 'Brisk Walking', 'Jogging (Outdoors)', or 'Walking'. NEVER output 'Treadmill'.` : ''}
